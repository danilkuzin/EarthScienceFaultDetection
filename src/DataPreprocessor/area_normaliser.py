...
class AreaNormaliser:
    ...
    def normalise(self, landsat: bool = False):
        associated_regions = []
        ...
        if not landsat:
            ...
            mean_elevation = np.mean(elevations_array[elevations_array > 0])
            std_elevation = np.std(elevations_array[elevations_array > 0])
